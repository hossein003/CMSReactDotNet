...
export default memo(function Session() {
  const { productName, sessionID } = useParams();
  const [session, setSession] = useState({})
  const [sessions, setSessions] = useState([])

  useEffect(() => {
    fetch(`http://localhost:4000/v1/courses/${productName}/${sessionID}`, {
      headers: {
                ...
                <ul class="sidebar-topics__list">
                  {
                    sessions.map(session => (
                      <Link to={`/${productName}/${session._id}`}>
              ...
              <div class="episode-header__right">
                <a class="episode-header__right-back-link" href="#">
                  ...
                  <div class="episode-header__right-home">
                    <Link class="episode-header__right-home-link" to={`/product-info/${productName}`}>
                      به محصول خانه بروید
                    </Link>
